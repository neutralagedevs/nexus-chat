...
const useChat = () => {
    ...
    const [onTransactionReady, setOnTransactionReady] = useState<((data: TransactionData) => void) | null>(null);

    const getInitialSuggestedActions = useCallback(() => {
        if (connection.isConnected) {
            return [
                { id: 'portfolio', type: 'check_portfolio' as const, label: 'Check Portfolio' },
                { id: 'convert', type: 'confirm_fiat' as const, label: 'Convert to Fiat' },
...
I'm your AI specialist for seamless USDT stablecoin conversions. I can help you:

**Convert USDT to local currency** (USDT → NGN, USD, EUR)
**Get real-time USDT market rates** and conversion estimates  
**Setup secure bank transfers** with industry-leading security
**Track transactions** from start to completion
**Optimize timing** for better conversion rates

${connection.isConnected
                ? `**Wallet Connected**: Ready to check your USDT portfolio and start conversions!`
                : `**Connect your wallet** to get started with personalized USDT portfolio analysis.`
            }

What would you like to do today? I'm here to make your USDT-to-fiat journey smooth and profitable!`,
        timestamp: new Date(),
        metadata: {
    ...
    const sendMessage = useCallback(async (content: string) => {
        const isCancellation = /cancel|stop|no thanks|nevermind|abort/i.test(content);
        if (isCancellation) {
        ...
        try {
            ...
            const analysis = await aiAssistant.analyzeUserMessage(content, conversationContext);
            ...
            const hasMinimalTransactionData = pendingTransactionData &&
                (pendingTransactionData.tokenIn || pendingTransactionData.amountIn || pendingTransactionData.fiatAmount);

            const shouldAutoTrigger = !conversationState.hasUserCancelled &&
                ((newMessageCount >= 5) ||
                    (hasMinimalTransactionData && newMessageCount >= 3 && analysis.intent === 'fiat_conversion'));
            ...
            if (newMessageCount >= 3 && hasMinimalTransactionData && !conversationState.hasUserCancelled) {
                enhancedResponse += `

**Ready to Proceed**: I have the details needed for your conversion. Let me set this up for you to review and sign.`;
            } else if (newMessageCount >= 4 && !hasMinimalTransactionData && !conversationState.hasUserCancelled) {
                enhancedResponse += `
...
**Let's Complete This**: To proceed with your conversion, I'll need either the token amount you want to convert or your desired fiat amount. Once I have that, we can proceed to the secure signing process.`;
            ...
            if (analysis.intent === 'fiat_conversion' && analysis.extractedData?.tokenIn) {
...
**Market Context**: I'll check current ${tokenSymbol} rates to ensure you get the best conversion value.`;
            ...
            const shouldShowTransactionData = analysis.intent === 'fiat_conversion'
                && analysis.extractedData
            ...
            const assistantMessage: ChatMessage = {
                ...
                metadata: {
                    ...
                    confirmationRequired: analysis.intent === 'fiat_conversion' || shouldTriggerTransaction,
            ...
            if (shouldTriggerTransaction && pendingTransactionData && onTransactionReady) {
                setTimeout(() => {
    ...
    const setTransactionReadyCallback = useCallback((callback: (data: TransactionData) => void) => {
...
function generateSuggestedActions(
    ...
    if (analysis.intent === 'portfolio') {
        if (!isConnected) {
            actions.push({ id: 'connect_portfolio', type: 'connect_wallet' as const, label: 'Connect for Portfolio' });
        } else {
            actions.push(
                { id: 'view_portfolio', type: 'check_portfolio' as const, label: 'View Portfolio' },
                { id: 'convert_options', type: 'confirm_fiat' as const, label: 'Conversion Options' }
            );
        }
    }
    ...
    if (analysis.intent === 'query') {
        ...
        const hasConversionKeywords = ['convert', 'cash', 'fiat', 'bank', 'withdraw', 'sell'].some(keyword =>
            content.includes(keyword)
        );

        if (hasConversionKeywords) {
            actions.push(
                { id: 'start_conversion', type: 'confirm_fiat' as const, label: 'Start Conversion' },
                { id: 'market_info', type: 'market_rates' as const, label: 'Market Info' }
            );
        } else {
            actions.push(
                { id: 'convert_crypto', type: 'confirm_fiat' as const, label: 'Convert Crypto' }
            );

            if (isConnected) {
                actions.push({ id: 'portfolio_check', type: 'check_portfolio' as const, label: 'Check Portfolio' });
            }

            actions.push({ id: 'learn_process', type: 'learn_more' as const, label: 'Learn Process' });
        }
    }
    ...
    if (analysis.intent === 'technical_support') {
        actions.push(
            ...
            { id: 'check_status', type: 'check_portfolio' as const, label: 'Check Status' }
        );
    }

    if (analysis.intent === 'unknown') {
        actions.push(
            { id: 'explore_conversion', type: 'confirm_fiat' as const, label: 'Explore Conversions' },
            { id: 'how_it_works', type: 'learn_more' as const, label: 'How It Works' }
        );
    }
